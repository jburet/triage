...
- **`diff_analysis` has no entrypoint, and no image has ever been built.**
  ...
  sub-graph handles that correctly — an unknown in the diagnosis and a confidence cap. The
  container image exists as of M7 phase 3 (`docker/analysis/Dockerfile`) and has run both
  investigative kinds live against real Zeenea repositories; what it is not is *published*,
  so no cluster can pull it and `config.analysis.job.image` is still empty.
