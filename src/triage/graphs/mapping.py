"""The service-mapping graph: which repository each running service is (M6).
    ...
    services | nothing
      → select_services → derive_workloads → persist_workloads → report_mapping → END
...
from triage.nodes.mapping import (
    derive_workloads,
    persist_workloads,
    report_mapping,
    select_services,
...
def build_graph() -> MappingGraph:
    ...
    builder.add_node("report_mapping", report_mapping)
    ...
    builder.add_edge("persist_workloads", "report_mapping")
    builder.add_edge("report_mapping", END)
